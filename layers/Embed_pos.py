...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, x, start_index=0):
        length_needed = x.size(1) 
        end_index = start_index + length_needed
        return self.pe[:, start_index:end_index]
...
class PatchEmbedding_C_group(nn.Module):
    ...
    def forward(self, x, start_index=0):
        ...
        out = []
        for index in range(len(self.feature_group)):  
            group_tensor = x[:,:,:,self.feature_group[index]]
            reshaped_tensor = torch.reshape(group_tensor, (group_tensor.shape[0], group_tensor.shape[2], group_tensor.shape[3] * group_tensor.shape[1]))
            out.append(reshaped_tensor)

            pos_emb = self.position_embedding(out[index], start_index=start_index)
            out[index] = self.value_embedding[index](out[index]) + pos_emb
            out[index] = self.dropout(out[index])
